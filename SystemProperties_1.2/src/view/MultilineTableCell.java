...
public class MultilineTableCell 
...

    {
        ...
        map.put(TextAttribute.FONT, new Font(Font.SERIF,Font.PLAIN,14));
        ...
        public void paintComponent(Graphics gr) {
            ...
            if ( text != null && !text.isEmpty() ) {
                ...
                if (lineMeasurer == null) {
                    AttributedCharacterIterator paragraph = new AttributedString(text,map).getIterator();
                    paragraphStart = paragraph.getBeginIndex();
                    paragraphEnd = paragraph.getEndIndex();
                    FontRenderContext frc = g.getFontRenderContext();
                    lineMeasurer = new LineBreakMeasurer(paragraph,BreakIterator.getWordInstance(), frc);
                }
                float breakWidth = ((float)table.getColumnModel().getColumn(columnIndex).getWidth())-8;
                float drawPosY = 0;
                ...
                while (lineMeasurer.getPosition() < paragraphEnd) {
                    ...
                    float drawPosX = layout.isLeftToRight()
                        ? 4 : breakWidth - layout.getAdvance();
                    // Move y-coordinate by the ascent of the layout.
                    drawPosY += layout.getAscent()+2;
                    // Draw the TextLayout at (drawPosX, drawPosY).
                    layout.draw(g, drawPosX, drawPosY);
                    // Move y-coordinate in preparation for next layout.
                    drawPosY += layout.getDescent()+ 4 + layout.getLeading();
                }
